...
Используемым событием в нашей программе являются ACTION события.
	 */
public class Interface_Otp extends JDialog {
	...
	*/
	...
	public static void main(String[] args) {
		...
		*/
		try {
			Interface_Otp dial = new Interface_Otp();
...
короткое описание возникшей проблемы, а его метод  getMessage()возвращает это описание.
	 ...
	 * Create the dialog.
	 */
	public static JLabel Label_sr_dn_otp, Label_otp;
	...
	- классы
  	...
  	что переменные --Label_sr_dn_otp, Label_otp-- доступны любому классу.
  	JLabel - С помощью JLabel можно показать текст с иконкой.
*/	
	public Interface_Otp() {
		 ...
		 */
		Label_1.setToolTipText("<html>Заработная плата за предыдущий<br> год / кол-во рабочих дней в году</html>");//всплывающая подсказка написанная с помощью html кода. br - переход на след.строку
		panel.add(Label_1);//добавить на панель, add - метод добавления компоненты в контейнер.
				
		Label_sr_dn_otp = new JLabel("");//сюда будет записываться рассчитанный средний дневной заработок
		panel.add(Label_sr_dn_otp);//добавить на панель, add - метод добавления компоненты в контейнер.

		JLabel Label_2 = new JLabel("рублей");
		panel.add(Label_2);//добавить на панель, add - метод добавления компоненты в контейнер.
				
		JLabel Label_3 = new JLabel("2. Сумма отпускных к выдаче:");
		Label_3.setToolTipText("Сумма, которая выдается сотруднику ");
		panel.add(Label_3);//добавить на панель, add - метод добавления компоненты в контейнер.
		
		Label_otp = new JLabel("");//сюда будет записываться рассчитанные отпускные
		panel.add(Label_otp);//добавить на панель, add - метод добавления компоненты в контейнер.
